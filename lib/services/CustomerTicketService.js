...
import Urgency from "../database/models/Urgency.js";
...
export async function getTicketDetails(ticketId) {
 ...
 async function deleteTicketAttachment(ticketId) {
  Promise.allSettled([await sequelize.query(
    `DELETE FROM ticket_attach WHERE ticket_id = :ticketId;`,
    {
      ...
      await sequelize.query(
        `DELETE FROM reply_tickets WHERE ticket_id= :ticketId;`,
        {
          replacements: { ticketId },
          type: sequelize.QueryTypes.DELETE,  
        }
...
export async function deleteTicketWithAttachment(ticket) {
  console.log("Deleting ticket with attachment:", ticket.ticketno);
    await deleteTicketAttachment(ticket.ticketno);
    await deleteTicket(ticket.ticketno);
} 
...
export async function getAllFAQsByKeyword(keyword) {
  const searchKeyword = `%${keyword}%`;
  const results= await sequelize.query(
    `SELECT f.id as faqId, f.question as question, f.answer as answer, TO_CHAR(f.created_at, 'YYYY-MM-DD HH24:MI') AS createdAt
     FROM faq f
     WHERE LOWER(f.question) LIKE LOWER(:searchKeyword)
        OR LOWER(f.answer) LIKE LOWER(:searchKeyword);`,
    {
